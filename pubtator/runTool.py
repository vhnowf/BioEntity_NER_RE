...
def splitBioc(inBioc, outDir, maxLength, stripAnnotations=False):
    ...
    with biocxml.iterparse(inBioc) as parser, biocxml.iterwrite(docName) as writer:
...
def mergeBiocWithMetadata(metaDir, inDir, outBioc):
    ...
    with biocxml.iterwrite(outBioc) as writer:
        for filename in filenames:
            ...
            with biocxml.iterparse(inBioc) as inParser, biocxml.iterparse(metaBioc) as metaParser:
...
def mergeBioc(inDir, outBioc):
    ...
    with biocxml.iterwrite(outBioc) as writer:
        for inBioc in inBiocs:
            ...
            with  biocxml.iterparse(inBioc) as parser:
                parser = bioc.BioCXMLDocumentReader(f)
                for doc in parser:
                    writer.write_document(doc)
